...
<div class="space-y-6">
    ...
    <?php elseif ($page === 'payments'): ?>
        ...
        <div class="flex flex-wrap gap-2 mb-6">
            <a href="?page=payments&status=all" class="px-4 py-2 rounded-lg transition <?= $currentFilter === 'all' ? 'bg-blue-600 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-gray-600' ?>">
                All
            </a>
            <a href="?page=payments&status=pending" class="px-4 py-2 rounded-lg transition <?= $currentFilter === 'pending' ? 'bg-yellow-600 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-gray-600' ?>">
                Pending
            </a>
            <a href="?page=payments&status=approved" class="px-4 py-2 rounded-lg transition <?= $currentFilter === 'approved' ? 'bg-green-600 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-gray-600' ?>">
                Approved
            </a>
            <a href="?page=payments&status=rejected" class="px-4 py-2 rounded-lg transition <?= $currentFilter === 'rejected' ? 'bg-red-600 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-gray-600' ?>">
                Rejected
            </a>
        </div>
                ...
                <table class="w-full text-sm">
                    <thead class="bg-gray-50 dark:bg-gray-900/50">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase">Payment Method</th>
                            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase">Transaction Reference</th>
                            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase">Book</th>
                            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase">Submitted At</th>
                        ...
                        <?php if (!empty($payments) && is_array($payments)): ?>
                            <?php foreach ($payments as $payment): ?>
                                <?php
                                    ...
                                    // ✅ Debug line – uncomment to check values in error log
                                    // error_log("🔍 Payment ID: $id, Status: '$status', Refund: '$refundStatus'");
                                ...
                                <tr class="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-900/30 transition">
                                    ...
                                    <td class="px-4 py-3 text-gray-700 dark:text-gray-300">Loan #<?= $payment['loan_id'] ?? '?' ?></td>
                                    <td class="px-4 py-3 text-gray-700 dark:text-gray-300"><?= number_format($amount, 2) ?> MMK</td>
                                    <td class="px-4 py-3">
                                    ...
                                    <td class="px-4 py-3 text-gray-700 dark:text-gray-300"><?= htmlspecialchars($transactionRef) ?></td>
                                    <td class="px-4 py-3 text-gray-700 dark:text-gray-300"><?= htmlspecialchars($bookTitle) ?></td>
                                    <td class="px-4 py-3 text-gray-700 dark:text-gray-300">
                                    ...
                                    <td class="px-4 py-3 text-center space-x-1">
                                        <?php if ($status === 'pending_approval'): ?>
                                            <form action="<?= BASE_URL ?>/librarian/payments/<?= $id ?>/approve" method="POST" class="inline">
                                                <button type="submit" class="text-green-600 hover:text-green-800 dark:text-green-400 dark:hover:text-green-300" title="Approve" onclick="return confirm('Approve this payment?')">
                                                    <i class="fas fa-check-circle"></i>
                                                </button>
                                            </form>
                                            <form action="<?= BASE_URL ?>/librarian/payments/<?= $id ?>/reject" method="POST" class="inline">
                                                <button type="submit" class="text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300" title="Reject" onclick="return confirm('Reject this payment?')">
                                                    <i class="fas fa-times-circle"></i>
                                                </button>
                                            </form>
                                        <?php endif; ?>
                                        ...
                                        <?php if (($status === 'approved' || $status === 'completed') && $refundStatus === 'none'): ?>
                                            <a href="<?= BASE_URL ?>/librarian/payments/<?= $id ?>/refund" class="text-purple-600 hover:text-purple-800 dark:text-purple-400 dark:hover:text-purple-300" title="Refund">
                                                <i class="fas fa-undo-alt"></i>
                                            </a>
                                        <?php endif; ?>

                                        <?php if ($status === 'approved' || $status === 'completed'): ?>
                                            <a href="<?= BASE_URL ?>/librarian/payments/invoice/<?= $id ?>" class="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300" title="Invoice">
                                                <i class="fas fa-file-invoice"></i>
                                            </a>
                                        <?php endif; ?>

                                        <a href="<?= BASE_URL ?>/librarian/payments/<?= $id ?>" class="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300" title="Details">
